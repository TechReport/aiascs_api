  ...
  login: async (req, res, next) => {
    try {
      ...
      if (user.firstTimeLoginStatus === 0) {
        console.log('hi');
        let tempToken = jwt.sign({ id: user._id, email: user.email, displayName }, process.env.JWT_SECRET, { expiresIn: 20 * 60 });
 ...
 id: user._id, email: user.email, displayName, roleId: user.role._id 
  ...
  register: async (req, res, next) => {
    try {
      ...
      let newUser = await registerUser(req.body.newUser);
      ...
      if (e.code === 11000) {
        const target =
          Object.keys(e.keyValue)[0] === 'phoneNumber'
          ? 'Phone number'
          : 'Email address';
